(() => {
  ...
  const card = item => {
    ...
    if ((item.sport || structure.sport) === 'running') details.append(field('最大距離（km・任意）', 'maximum_distance_km', structure.maximum_distance_km, 'number', 'AIはこの距離を超えません。'), field('最速ペース上限（秒/km・任意）', 'fastest_pace_seconds_per_km', structure.fastest_pace_seconds_per_km, 'number', '例: 330 は 5:30/km。AIはこれより速く設定しません。'));
  ...
  const collect = () => [...document.querySelectorAll('.workout-candidate')].map(card => { const value = key => card.querySelector(`[data-key="${key}"]`)?.value ?? ''; const sport = value('sport') || candidates.find(item => item.id === card.dataset.id)?.sport || 'running'; return { id: card.dataset.id || null, sport, title: value('title').trim(), description: value('description').trim(), minimum_minutes: Number(value('minimum_minutes')), maximum_distance_km: numberOrNull(value('maximum_distance_km')), fastest_pace_seconds_per_km: numberOrNull(value('fastest_pace_seconds_per_km')), maximum_duration_minutes: numberOrNull(value('maximum_duration_minutes')), example_structure: value('example_structure').trim(), required_environment_keywords: [...card.querySelectorAll('[data-environment]:checked')].map(input => input.dataset.environment) }; });
  const install = () => {
