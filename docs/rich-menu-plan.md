...
| 位置 | 表示 | action | 初期動作 |
|---|---|---|---|
| 上段左 | 練習メニュー | `training_menu` | 既存計画を署名付きURLで表示し、なければ初回承認用の安全な週間案を作成して開く |
...
| target | RM-03での応答 |
|---|---|
| `training_menu` | 署名・期限付きURLで練習メニュー画面を開く。pendingまたはactive planを表示し、どちらもなければ初回承認用の安全な週間案を作成する。画面は今日のメニュー、今週の予定、最近の実績をこの順で表示 |
...
PF-01、AC-01、PL-01C、PL-01D、PL-01E、PL-01F、PL-01G、MA-01、WT-01は完了している。上段左はRM-06として「練習メニュー」へ更新済みであり、PL-01Gで計画未作成ユーザーにも初回案を表示できる。次の推奨セッションはNT-01（定期実行と運用）である。メニュー画像へ「体重」領域を追加する場合は別のRM変更として扱う。
