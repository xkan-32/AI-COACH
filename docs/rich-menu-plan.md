...
PF-01は完了している。次の推奨セッションはAC-01（Activity/Laps/Streams）であり、詳細なActivity履歴を後続の計画・分析・AI拡張で利用できるデータ基盤を整える。メニュー画像へ「体重」等の新領域を追加する判断はWT-01のUI設計時に行い、それまでは6領域の座標・画像を変更しない。
