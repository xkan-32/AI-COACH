...
| `goals` | 有効な目標一覧を表示し、追加・変更・無効化をQuick Replyで選択 |
| `progress` | 準備中と既存フィードバックを案内 |
| `settings` | 目標・運動環境の設定入口をQuick Replyで表示 |

PF-01では操作コマンドやIDの手入力を通常導線にしない。目標の操作、主目標／副目標、目標種別、登録済み項目、運動環境の既知区分はpostback Quick Replyで選択する。目標内容には種別ごとの入力例を表示する。運動環境の追加は複数選択modeとし、場所・種目と器具を行き来し、再タップで解除してから「選択完了」で一括保存する。目標内容、日付、未定義の運動環境など自由記述が必要な値だけをテキスト入力する。既存テキストコマンドは後方互換用として維持する。

運動環境の変更も登録済み項目、変更後の種類、既知項目を順にQuick Replyで選ぶ。通常導線ではIDや既知の場所・種目・器具名を入力させず、「その他」を選んだ場合だけ新しい表示名を自由入力する。
...
次セッションはPF-01を推奨する。現在の`settings` actionを、既存の「運動環境確認」案内から構造化された一覧・編集会話の開始へ拡張する。メニュー画像へ「体重」等の新領域を追加する判断はWT-01のUI設計時に行い、PF-01では6領域の座標・画像を変更しない。
