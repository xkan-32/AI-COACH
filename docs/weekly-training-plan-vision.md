...
正式入口としてリッチメニュー上段左を「練習メニュー」に割り当てる。既存の6領域レイアウトは維持し、この入口は署名・期限付きURLで同一originの練習メニュー画面を開く。画面は今日のメニュー、今週の予定、最近の実績をこの順に表示する。実績一覧へGPS、生stream、route hash、Description、健康自由記述は返さない。
...
実装済み。変更要求・変更案・決定履歴の分離、3種類の変更範囲、署名付き承認、別案revision、安全block維持は`docs/next-session-pl-01f.md`を参照する。
