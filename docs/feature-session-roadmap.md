...
- WT-01（体重記録。LINE会話で登録・同日訂正・7日/30日平均・目標差）
- PL-01D（Strava／手動Activityの共通matcher、曖昧・計画外・重複・未実施確認・手動訂正）

未実装・拡張対象:

- 状態別リッチメニュー（RM-05）
- timezone別Scheduler、通知、再通知、DLQ、監視（NT-01）
- 7日/30日負荷、連続症状分析
- 提案修正、進捗表示、通知設定
...
実装記録は`docs/next-session-wt-01.md`を参照する。

- 日付と体重の登録・訂正、7日/30日平均、目標差を実装する。
- 単位kg、妥当範囲、同日訂正をappend-onlyログで扱う。
- 生値と集計値を分け、体重数値をlogへ出さない。
- リッチメニュー画像は変更せず、「体調を記録」ハブと数字入力から開始する。
