...
実装判断: 直接編集は安全上、元の計画より負荷を上げない変更に限定する。取消は元の`PlannedWorkout`を削除せず、`WorkoutExecutionState.cancelled`と、休養へ置き換えた新しいactive plan versionを保存する。種目・環境を増やす変更はAI代替案の承認フローへ委ねる。
