...
async def test_manual_activity_is_saved_without_strava_fields() -> None:
    ...
    assert saved.athlete_id == "line-1"
    ...
    assert messenger.texts[-1][1].startswith("運動を記録しました。")
    assert "Stravaへはまだ投稿していません" in messenger.texts[-1][1]
